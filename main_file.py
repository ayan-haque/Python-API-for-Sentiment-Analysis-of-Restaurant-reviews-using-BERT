...
class  Restaurant_Reviews:
	...
	def predict(self, df):
		...
		print('The shape of Sample_data_2d: ', np.shape(sample_data_2d))   # sample_data_2d is BERT emd from FLASK
		...
		for i_emd_p in RR_Positive_dataset_emd:
			score_matrix_p = cdist(sample_data_2d, i_emd_p, 'cosine')   # i_emd_p is BERT emd of each ith positive review
		...
		for i_emd_n in RR_Negative_dataset_emd:
			score_matrix_n = cdist(sample_data_2d, i_emd_n, 'cosine')    # i_emd_n is BERT emd of each ith negative review  # 0.05
			score_matrix_n = 1 - score_matrix_n  # Here I'm increasing the dist value between similar reviews 
